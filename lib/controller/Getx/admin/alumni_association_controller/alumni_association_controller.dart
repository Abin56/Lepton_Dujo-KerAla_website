...
class AlumniController extends GetxController {
  ...
  Future<void> addAlumniMember(BuildContext context) async {
    if (nameController.text.isNotEmpty &&
        positionController.text.isNotEmpty &&
        designationController.text.isNotEmpty) {
      try {
        final data = AlumniAssociationModel(
        ...
        DocumentReference<Map<String, dynamic>> result =
            await firebaseFirestore.add(data.toMap());
        ...
        if (imageDataUin8.value != null) {
          ...
          final filePath = await firebaseStorage
              .ref(
                  'files/${Get.find<AdminLoginScreenController>().schoolID}/${Get.find<GetFireBaseData>().bYear.value}/Alumni/$id')
  ...
  Future<void> updateAlumniDetails(
    String memberId,
    ...
    if (nameController.text.isNotEmpty &&
        ...
        designationController.text.isNotEmpty) {
      try {
        AlumniAssociationModel studentData = AlumniAssociationModel(
          name: nameController.text,
          position: positionController.text,
          designation: designationController.text,
          id: memberId,
        ...
        if (imageDataUin8.value != null) {
          final filePath = await firebaseStorage
              .ref(
                  'files/${Get.find<AdminLoginScreenController>().schoolID}/${Get.find<GetFireBaseData>().bYear.value}/Alumni/$imageId')
              .putData(imageDataUin8.value!);
